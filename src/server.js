  ...
  try {
    ...
    io.use((socket, next) => {
      try {
        ...
        const token = parsedCookies.token; // cookie name
        if (!token) return next(new Error('No token found'));

        const user = jwt.verify(token, process.env.JWT_SECRET); // decode JWT
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('sendMessage', async (payload, cb) => {
        try {
          ...
          socket.emit('messageSent', { conversationId: String(conversation._id), message });
          if (typeof cb === 'function') cb({ ok: true, conversationId: String(conversation._id), message });

          const receiverSocketId = onlineUsers.get(String(receiverId));
          if (receiverSocketId) {
            io.to(receiverSocketId).emit('receiveMessage', { conversationId: String(conversation._id), message });
          }
        } catch (err) {
